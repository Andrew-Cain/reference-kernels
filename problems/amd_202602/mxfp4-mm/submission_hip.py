...
_INLINE_KERNEL_SOURCE = None  # loaded from .hip file or set below

def _get_inline_source():
    """Read from .hip file on disk, cache as inline fallback."""
    ...
    if os.path.exists(hip_path):
        with open(hip_path) as f:
            return f.read()
    # Inline fallback: stripped kernel without comments (for server submission)
    return r"""
...
#if USE_SHUFFLE
#if MFMA_MODE == 0
    const int b_n_tile = n_sub >> 4;
    const int b_n_local = lane_row;
#else
    const int b_n_tile = (n_sub >> 4) + (lane_row >> 4);
    const int b_n_local = lane_row & 15;
#endif
    const long long b_tile_base = (long long)b_n_tile * K_packed * 16;
    const int b_n_off = b_n_local * 16;
#else
...
#endif
            a_reg, b_reg, acc, TYPE_FP4, TYPE_FP4, 0, scale_a_val, 0, scale_b_val);
    }
#if INTRA_K_SPLITS > 1
    __shared__ float lds_partial[LDS_TOTAL];
    if (k_wave > 0) {
        const int lds_base = (k_wave - 1) * N_WAVES * LDS_ELEMS_PER_NWAVE
                           + n_wave * LDS_ELEMS_PER_NWAVE + lane_id * ACC_PER_KG;
    ...
    if (c_col < N) {
        for (int j = 0; j < ACC_PER_KG; j++) {
            ...
            if (c_row < M) {
                ...
                if (NUM_KSPLIT > 1) atomicAdd(c_ptr, acc[j]);
                else *c_ptr = acc[j];
            }
        }
    }
...
KERNEL_SOURCE = _get_inline_source()
...
def _compile_kernel(mfma_mode=0, intra_k_splits=1, use_shuffle=0):
    key = (mfma_mode, intra_k_splits, use_shuffle)
    ...
    if err != 0:
        ...
        raise RuntimeError(f"HIPRTC compile failed (mode={mfma_mode}, iks={intra_k_splits}, shuf={use_shuffle}, err={err}):\n{log_buf.value.decode()}")
...
def mxfp4_gemm_hip(A_q, B_q, A_scale, B_scale, M, N, K,
    ...
    func = _compile_kernel(mfma_mode, intra_k_splits, use_shuffle)
    _launch_kernel(
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # (M, N, K) -> (num_ksplit, block_k, mfma_mode, intra_k_splits, use_shuffle)
    ...
    _SHAPE_CONFIGS = {
        #                     ksplit  blk_k  mode  iks  shuf
        (4, 2880, 512):     (1,     64,    0,    4,   1),
        (16, 2112, 7168):   (16,    32,    1,    1,   0),   # shuffle hurts 32x32 mode
        (32, 4096, 512):    (1,     64,    0,    4,   1),
        (32, 2880, 512):    (1,     64,    0,    4,   1),
        (64, 7168, 2048):   (1,     64,    0,    2,   1),
        (256, 3072, 1536):  (1,     64,    0,    2,   1),
    }

    cfg = _SHAPE_CONFIGS.get((m, n, k))
    if cfg is not None:
        num_ksplit, block_k, mfma_mode, intra_k_splits, use_shuffle = cfg
    else:
        mfma_mode = 0
        intra_k_splits = 1
        use_shuffle = 0
    ...
    return mxfp4_gemm_hip(
        A_q.view(torch.uint8), B_data.view(torch.uint8), A_scale, B_scale, m, n, k,
